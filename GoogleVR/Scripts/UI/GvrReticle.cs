...
public class GvrReticle : MonoBehaviour, IGvrGazePointer {
  ...
  public int reticleSegments = 20;          // 非み碭娩︽

  /// Growth speed multiplier for the reticle/
  public float reticleGrowthSpeed = 8.0f;   // 窱砰非み硉

  // Private members
  private Material materialComp;
  private GameObject targetObj;             // ヘ夹砰
  ...
  private float reticleDistanceInMeters = 10.0f;    // 非み籔禯瞒
  ...
  // (in degrees).
  private const float kReticleGrowthAngle = 1.2f;

  // Minimum distance of the reticle (in meters).
  private const float kReticleDistanceMin = 0.45f;  // 非み程禯瞒
  // Maximum distance of the reticle (in meters).
  private const float kReticleDistanceMax = 10.0f;  // 非み程环禯瞒
  ...
  void Update() {
    UpdateDiameters();
    //Debug.Log(reticleDistanceInMeters);
  }
  ...
  private void UpdateDiameters() {
    ...
    materialComp.SetFloat("_InnerDiameter", reticleInnerDiameter * reticleDistanceInMeters);    // 非みず伴畖
    materialComp.SetFloat("_OuterDiameter", reticleOuterDiameter * reticleDistanceInMeters);    // 非み伴畖
    materialComp.SetFloat("_DistanceInMeters", reticleDistanceInMeters);                        // 非み籔禯瞒
  }
